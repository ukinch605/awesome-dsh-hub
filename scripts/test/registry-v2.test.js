...
test('legacy removal events are explicitly marked unconfirmed without touching other events', () => {
  const ledger = { schemaVersion: 1, events: [
    { id: 'r', type: 'plugin_removed', changes: { repo: 'a/b' } },
    { id: 'v', type: 'package_version_changed', changes: { from: '1', to: '2' } },
  ] };
  const migrated = annotateLegacyRemovalEvents(ledger);
  assert.equal(migrated.events[0].confirmation, 'unconfirmed-legacy-discovery');
  assert.deepEqual(migrated.events[0].changes, ledger.events[0].changes);
  assert.deepEqual(migrated.events[1], ledger.events[1]);
});
