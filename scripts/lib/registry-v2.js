...
export function generateEvents(previous, current, occurredAt, { suppressMigration = true, confirmedRemovalIds = null } = {}) {
  ...
  for (const [id, old] of before) {
    if (!after.has(id) && (!confirmedRemovalIds || confirmedRemovalIds.has(id))) {
      emit('plugin_removed', id, { repo: old.repo, ...(confirmedRemovalIds ? { confirmation: 'direct-verification' } : {}) });
    }
  }
  return events;
}
...
export function annotateLegacyRemovalEvents(ledger) {
  return {
    ...ledger,
    // Keep the original hashed event body intact; this top-level annotation is
    // migration metadata, not a retroactive claim about the historical event.
    events: (ledger.events || []).map((event) => event.type === 'plugin_removed' && !event.confirmation && !event.changes?.confirmation
      ? { ...event, confirmation: 'unconfirmed-legacy-discovery' }
      : event),
  };
}
